...
class Solution:
    ...
    #     mp = {employee.id: employee for employee in employees} # list  => map
    #     total = 0
    #     que = collections.deque([id])
    #     while que:
    #         curIdx = que.popleft()
    #         employee = mp[curIdx]
    #         total += employee.importance # 累加重要度
    ...
    def getImportance(self, employees, id):
        # Write your code here.
        def dfs(id):
            count = G[id].importance # 累加重要度
            for sub in G[id].subordinates: # 循环  直系下属的 id 列表
                count += dfs(sub) # 累加下属的的重要度
            return count
        # imput list => map
        G = {}
        for e in employees:
