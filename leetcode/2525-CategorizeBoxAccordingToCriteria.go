...
func categorizeBox(length int, width int, height int, mass int) string {
    checkBulky := func (length, width, height int) bool {
        maxDim, maxVol := 10000, 1000000000
        if maxDim <= length || maxDim <= width || maxDim <= height || maxVol <= length * width * height { return true }
        return false
    }
    checkHeavy := func (mass int) bool {
        if mass >= 100 { return true }
        return false
    }
    bulky, heavy := checkBulky(length, width, height), checkHeavy(mass)
    switch {
...
// However mass >= 100, so the box is "Heavy".
// Since the box is not "Bulky" but "Heavy", we return "Heavy".
fmt.Println(categorizeBox(1000, 35, 700, 300)) // "Heavy"
// Example 2:
// Input: length = 200, width = 50, height = 800, mass = 50
// Output: "Neither"
// Explanation: 
...
// Its mass is also less than 100, so it cannot be categorized as "Heavy" either. 
// Since its neither of the two above categories, we return "Neither".
fmt.Println(categorizeBox(200, 50, 800, 50)) // "Neither"
}