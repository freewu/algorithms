...
func maximumNumber(num string, change []int) string {
    res := []byte(num)
    for i, v := range num {
        if change[v - '0'] > int(v - '0') {
            for ; i < len(res) && change[res[i] - '0'] >= int(res[i] - '0'); i++ {
                res[i] = byte(change[res[i] - '0']) + '0'
            }
            break
        }
    }
...
func main() {
    ...
    // Thus, "021" becomes "934".
    // "934" is the largest number that can be created, so return it.
    fmt.Println(maximumNumber("021", []int{9,4,3,5,7,2,1,9,0,6})) // "934"
    // Example 3:
    // Input: num = "5", change = [1,4,7,5,3,2,5,6,9,4]
    // Output: "5"
    // Explanation: "5" is already the largest number that can be created, so return it.
    fmt.Println(maximumNumber("5", []int{1,4,7,5,3,2,5,6,9,4})) // "5"
}