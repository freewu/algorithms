...
-- Write a solution to report the minimum score requirement for each school. If there are multiple score values satisfying the above conditions, choose the smallest one. 
...
-- 运用笛卡尔积选择申请人数在学校容量范围内的对应的最小分数
WITH t AS (
    SELECT 
        school_id, min(score) as score
    FROM 
        Schools
    CROSS JOIN 
        Exam
...
(
    SELECT school_id, score FROM t
)
UNION
(-- 需要合并上 -1 的数据
    SELECT 
        school_id, -1 as score 
    FROM 
        Schools
    WHERE 
        school_id NOT IN (SELECT school_id FROM t)
) 
