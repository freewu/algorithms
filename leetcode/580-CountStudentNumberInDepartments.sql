...
-- and number of students majoring in each department for all departments in the Department table (even ones with no current students).
...
SELECT
    d.dept_name,
    IFNULL(s.num ,0) AS student_number
FROM
    Department AS d
LEFT JOIN
    (
