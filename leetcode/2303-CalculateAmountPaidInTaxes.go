...
func calculateTax1(brackets [][]int, income int) float64 {
    ...
    for _, bracket := range brackets {
        upper, rate := bracket[0], bracket[1]
        res += max(0, min(income, upper) - last) * rate
        last = upper
    }
    return float64(res) / 100.0
}
...
func main() {
    ...
    // The tax rate for the three tax brackets is 50%, 10%, and 25%, respectively.
    // In total, you pay $3 * 50% + $4 * 10% + $3 * 25% = $2.65 in taxes.
    fmt.Println(calculateTax([][]int{{3,50},{7,10},{12,25}}, 10)) // 2.65000
    // Example 2:
    // Input: brackets = [[1,0],[4,25],[5,50]], income = 2
    // Output: 0.25000
    // Explanation:
    ...
    // The tax rate for the two tax brackets is 0% and 25%, respectively.
    // In total, you pay $1 * 0% + $1 * 25% = $0.25 in taxes.
    fmt.Println(calculateTax([][]int{{1,0},{4,25},{5,50}}, 2)) // 0.25000
    // Example 3:
    // Input: brackets = [[2,50]], income = 0
    // Output: 0.00000
    // Explanation:
    // You have no income to tax, so you have to pay a total of $0 in taxes.
    fmt.Println(calculateTax([][]int{{2,50}}, 0)) // 0.00000

    fmt.Println(calculateTax1([][]int{{3,50},{7,10},{12,25}}, 10)) // 2.65000
    fmt.Println(calculateTax1([][]int{{1,0},{4,25},{5,50}}, 2)) // 0.25000
    fmt.Println(calculateTax1([][]int{{2,50}}, 0)) // 0.00000
}