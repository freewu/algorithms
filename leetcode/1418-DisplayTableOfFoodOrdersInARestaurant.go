...
// More specifically orders[i]=[customerNamei,tableNumberi,foodItemi] where customerNamei is the name of the customer, tableNumberi is the table customer sit at, 
...
func displayTable(orders [][]string) [][]string {
    tables, food := []string{}, []string{}
    foodMap, orderMap := map[string]bool{}, map[string]map[string]int{}
    for _, order := range orders {
        table := order[1]
        for i := 2; i < len(order); i++ {
            foodItem := order[i]
            if _, ok := orderMap[table]; !ok {
                orderMap[table] = map[string]int{}
                tables = append(tables, table)
            }
            if v := foodMap[foodItem]; !v {
                food = append(food, foodItem)
                foodMap[foodItem] = true
            }
            orderMap[table][order[i]] += 1
        }
    }
    ...
    res := [][]string{ append([]string{"Table"}, food...), }
    for _, table := range tables {
        temp := []string{ table, }
        for _, foodItem := range food {
            temp = append(temp, strconv.Itoa(orderMap[table][foodItem]))
        }
        res = append(res, temp)
    }
    return res
...
func main() {
    ...
    fmt.Println(displayTable(orders1)) // [["Table","Beef Burrito","Ceviche","Fried Chicken","Water"],["3","0","2","1","0"],["5","0","1","0","1"],["10","1","0","0","0"]] 
    // Example 2:
    // Input: orders = [["James","12","Fried Chicken"],["Ratesh","12","Fried Chicken"],["Amadeus","12","Fried Chicken"],["Adam","1","Canadian Waffles"],["Brianna","1","Canadian Waffles"]]
    // Output: [["Table","Canadian Waffles","Fried Chicken"],["1","2","0"],["12","0","3"]] 
    // Explanation: 
    ...
    fmt.Println(displayTable(orders2)) // [["Table","Canadian Waffles","Fried Chicken"],["1","2","0"],["12","0","3"]] 
    // Example 3:
    // Input: orders = [["Laura","2","Bean Burrito"],["Jhon","2","Beef Burrito"],["Melissa","2","Soda"]]
    // Output: [["Table","Bean Burrito","Beef Burrito","Soda"],["2","1","1","1"]]
    orders3 := [][]string{
    ...
    fmt.Println(displayTable(orders3)) // [["Table","Bean Burrito","Beef Burrito","Soda"],["2","1","1","1"]]
}