...
func maxIncreaseKeepingSkyline(grid [][]int) int {
    res, cols,rows := 0, len(grid), len(grid[0])
    colMaxs,rowMaxs := make([]int, cols),make([]int, rows)
    min := func (x, y int) int { if x < y { return x; }; return y; }
    max := func (x, y int) int { if x > y { return x; }; return y; }
    for i := 0; i < rows; i++ {
        for j := 0; j < cols; j++ {
            rowMaxs[i] = max(grid[i][j], rowMaxs[i]) 
            colMaxs[j] = max(grid[i][j], colMaxs[j])
        }
    }
    for i := 0; i < rows; i++ {
        for j := 0; j < cols; j++ {
...
func main() {
    ...
    fmt.Println(maxIncreaseKeepingSkyline(grid1)) // 35
    // Example 2:
    // Input: grid = [[0,0,0],[0,0,0],[0,0,0]]
    // Output: 0
    // Explanation: Increasing the height of any building will result in the skyline changing.
    ...
    fmt.Println(maxIncreaseKeepingSkyline(grid2)) // 0
}