...
WITH tbl AS (
    SELECT  
        ...
        AVG(course_rating * 1.00) OVER(PARTITION BY user_id) AS average,
...
FROM
     tbl 
WHERE 
    cnt > 4 AND -- 完成 至少 5 门课程）
    average >= 4 AND -- 平均评分 4 分或以上
    second_course IS NOT NULL -- 有后续课程
GROUP BY  
    first_course, second_course  
ORDER BY 
    transition_count DESC ,first_course ASC ,second_course ASC -- 按课程对频率 降序 排列，若频率相同则按第一课程名称和第二课程名称 升序 排列
