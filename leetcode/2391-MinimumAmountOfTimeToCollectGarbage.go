...
func garbageCollection(garbage []string, travel []int) int { 
    collectionTime, gTravelTime, pTravelTime, mTravelTime, currCum := 0, 0, 0, 0, 0 // to track cumulative travel time for a truck that has to collect garbage from a house
    for i, gCollect := range garbage {
        if i != 0 {
            currCum += travel[i-1] 
        }
        collectionTime += len(gCollect) // each unit takes 1 minute to process 每种垃圾收都需要花费 1 分钟
...
func garbageCollection1(garbage []string, travel []int) int {
    ...
    res, n := 0, len(garbage)
    for i := range travel { // 记入路程上的时间
        res += travel[i] * 3
    }
    for i := range garbage {
        res += len(garbage[i]) // 记入回收垃圾的时间 
    }
    for _, ch := range []byte("MPG") {
        for i := n - 1; i > 0 && strings.IndexByte(garbage[i], ch) < 0; i-- { // 从最后一个房子开始检查，是否有某种垃圾
            res -= travel[i-1] // 如果没有的话，不需要回收这种垃圾，把多用的时间还回来
        }
    }
    return res
}
...
func main() {
    ...
    fmt.Println(garbageCollection1([]string{"G","P","GP","GG"},[]int{2,4,3})) // 21
    fmt.Println(garbageCollection1([]string{"MMM","PGM","GP"},[]int{3,10})) // 37
}