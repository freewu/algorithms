...
func canMeasureWater(jug1Capacity int, jug2Capacity int, targetCapacity int) bool {
    ...
    gcd = func(a int, b int) int { if a == 0 { return b; }; return gcd(b % a, a); }
    val := gcd(jug1Capacity, jug2Capacity)
    if val == 0 {
        return targetCapacity == 0
    }
    // 否则，记x与y的最大公约数为gcd，则可以获得的容量z只能为gcd的整数倍，且z <= x + y
    return (targetCapacity % val == 0 ) && (targetCapacity <= jug1Capacity + jug2Capacity)
}
