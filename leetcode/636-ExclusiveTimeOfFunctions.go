...
//     For example, "0:start:3" means a function call with function ID 0 started at the beginning of timestamp 3, and "1:end:2" means a function call with function ID 1 ended at the end of timestamp 2. Note that a function can be called multiple times, possibly recursively.
...
// Example 1:
// <img src="https://assets.leetcode.com/uploads/2019/04/05/diag1b.png" />
...
func exclusiveTime(n int, logs []string) []int {
    ...
    for _, v := range logs {
        ...
        if last := len(stack) - 1; "start" == splits[1] {
            // 计算执行单位
            res[stack[last]] += time - pre
            stack = append(stack, index)
            pre = time
        } else {
            res[stack[last]] += time - pre + 1
            stack = stack[:last]
            pre = time + 1 // 增加下标一位，避免重复计算结束区间
        }
    }
    return res
}

// stack // stack Wrong Answer 121 / 123
func exclusiveTime1(n int, logs []string) []int {
    type Log struct {
        id        int
        timestamp int
    }
    stack, res := make([]Log, len(logs)), make([]int, n)
    for _, v := range logs {
        tmp := strings.Split(v, ":")
        ...
        if tmp[1][0] == 's' {
            if len(stack) > 0 {
                res[stack[len(stack)-1].id] += timestamp - stack[len(stack)-1].timestamp
            }
            stack = append(stack, Log{id, timestamp})
        } else {
            res[id] += timestamp - stack[len(stack)-1].timestamp + 1
...
func main() {
    ...
    fmt.Println(exclusiveTime1(2,[]string{"0:start:0","1:start:2","1:end:5","0:end:6"})) // [3,4]
    fmt.Println(exclusiveTime1(1,[]string{"0:start:0","0:start:2","0:end:5","0:start:6","0:end:6","0:end:7"})) // [8]
    fmt.Println(exclusiveTime1(2,[]string{"0:start:0","0:start:2","0:end:5","1:start:6","1:end:6","0:end:7"})) // [7,1]
}