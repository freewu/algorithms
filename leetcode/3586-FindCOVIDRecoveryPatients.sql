...
WITH patient_first_positive AS (
    SELECT 
        patient_id, MIN(test_date) AS first_positive_date 
    FROM 
        covid_tests 
    WHERE
        result = 'Positive' 
...
patient_dates AS (
    SELECT 
        a.patient_id, 
        a.first_positive_date, 
        b.test_date 
    FROM 
        patient_first_positive a, covid_tests b
    WHERE 
        a.patient_id = b.patient_id AND 
        b.test_date > a.first_positive_date AND 
        b.result = 'Negative'
),
almost_done AS (
    SELECT 
        patient_id, 
        first_positive_date, 
        MIN(DATEDIFF(test_date, first_positive_date)) AS recovery_time 
    FROM 
        patient_dates 
    GROUP BY 
        patient_id, first_positive_date
)
...
SELECT 
    ...
    r.recovery_time 
FROM 
    patients AS p, almost_done AS r
WHERE 
