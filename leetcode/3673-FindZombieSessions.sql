...
SELECT 
    session_id, 
    user_id, 
    ROUND( TIMESTAMPDIFF(MINUTE, MIN(event_timestamp), MAX(event_timestamp)) , 0) AS session_duration_minutes, 
...
GROUP BY 
    session_id, user_id
HAVING 
    session_duration_minutes  > 30.0 AND -- The session duration is more than 30 minutes.
    scroll_count >= 5 AND -- Has at least 5 scroll events.
    (COUNT(CASE WHEN event_type = 'click' THEN 1 ELSE NULL END) / CAST( scroll_count AS FLOAT) ) < 0.2 AND -- The click-to-scroll ratio is less than 0.20 .
    COUNT(CASE WHEN event_type = 'purchase' THEN 1 ELSE NULL END) = 0 -- No purchases were made during the session.
ORDER BY 
    scroll_count DESC, session_id ASC -- the result table ordered by scroll_count in descending order, then by session_id in ascending order.
