...
func hIndex1(citations []int) int {
    ...
    for i, c := range citations {
        if c >= length - i { // 从这个开始,后面的都比这篇的引用高,且满足 至少发表了 h 篇论文，并且 至少 有 h 篇论文被引用次数大于等于 h 。
            return length - i
        }
    }
    return 0
...
func main() {
    ...
    fmt.Printf("hIndex1([]int{3,0,6,1,5}) = %v\n",hIndex1([]int{3,0,6,1,5})) // 3
    fmt.Printf("hIndex1([]int{1,3,1}) = %v\n",hIndex1([]int{1,3,1})) // 1
}
