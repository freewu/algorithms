...
// and the total space wasted is the sum of the space wasted across every time t where 0 <= t < nums.length.
...
func minSpaceWastedKResizing(nums []int, k int) int {
    ...
    curr, sofar, currMax, val := 0, 0, 0, 0
    prefix := []int{0}
    for i, v := range nums {
        sofar += v
        currMax = max(currMax, v)
        prefix = append(prefix, currMax * (i + 1) - sofar)
    }
    dp[0] = prefix
    for r := 1; r <= k; r++ {
        for i := r + 1; i <= n; i++ {
            ...
            for j := i; j <= n; j++ {
                ...
                val = currMax*(j-i+1) - sofar
                dp[r][j] = min(dp[r][j], dp[r-1][i-1] + val)
            }
        }
    }
    return dp[k][n]
...
func main() {
    ...
    // We can set the initial size to be 20.
    // The total wasted space is (20 - 10) + (20 - 20) = 10.
    fmt.Println(minSpaceWastedKResizing([]int{10,20}, 0)) // 10
    // Example 2:
    // Input: nums = [10,20,30], k = 1
    // Output: 10
    // Explanation: size = [20,20,30].
    // We can set the initial size to be 20 and resize to 30 at time 2. 
    // The total wasted space is (20 - 10) + (20 - 20) + (30 - 30) = 10.
    fmt.Println(minSpaceWastedKResizing([]int{10,20,30}, 1)) // 10
    // Example 3:
    // Input: nums = [10,20,15,30,20], k = 2
    // Output: 15
    // Explanation: size = [10,20,20,30,30].
    // We can set the initial size to 10, resize to 20 at time 1, and resize to 30 at time 3.
    // The total wasted space is (10 - 10) + (20 - 20) + (20 - 15) + (30 - 30) + (30 - 20) = 15.
    fmt.Println(minSpaceWastedKResizing([]int{10,20,15,30,20}, 2)) // 15
}