...
func digitSum(s string, k int) string {
    merge := func (s string, k int) string {
        res, part := "", int(s[0] - '0')
        for i := 1; i < len(s); i++ {
            if i % k == 0 {
                res += strconv.Itoa(part)
                part = 0
            }
            part += int(s[i] - '0')
        }
        return res + strconv.Itoa(part)
    }
    for len(s) > k {
        s = merge(s, k)
    }
...
func digitSum1(s string, k int) string {
    min := func (x, y int) int { if x < y { return x; }; return y; }
    for len(s) > k {
        ns, n := "", len(s)
        m := (n + k - 1) / k
        for i := 0; i < m; i++ { // 将s分成m份
            // i*k, min((i+1)*k,n)
            // 左闭，右开
            t, val := s[i * k: min((i+1) * k,n)], 0
            for _, c := range t {
...
func main() {
    ...
    //   So, s becomes "13" + "5" = "135" after second round. 
    // Now, s.length <= k, so we return "135" as the answer.
    fmt.Println(digitSum("11111222223", 3)) // "135"
    // Example 2:
    // Input: s = "00000000", k = 3
    // Output: "000"
    // Explanation: 
    ...
    // Then we calculate the digit sum of each group: 0 + 0 + 0 = 0, 0 + 0 + 0 = 0, and 0 + 0 = 0. 
    // s becomes "0" + "0" + "0" = "000", whose length is equal to k, so we return "000".
    fmt.Println(digitSum("00000000", 3)) // "000"

    fmt.Println(digitSum1("11111222223", 3)) // "135"
    fmt.Println(digitSum1("00000000", 3)) // "000"
}