...
func eventualSafeNodes1(graph [][]int) []int {
    // 存在环的时候，不能完成所有课程
    n := len(graph)
    res, outdegree, indegree := []int{}, make([]int, n), make([][]int, n)
    ...
    queue := []int{}
    for i, v := range outdegree { // 不指向任何节点的， 可以入队列
        if v == 0 { // 终端节点
...
func eventualSafeNodes2(graph [][]int) []int {
    n := len(graph)
    res, nodes, visited := make([]int, 0, n), make([]bool, n), make([]bool, n)
    var dfs func(node int) bool
    dfs = func(node int) bool {
        if len(graph[node]) == 0 { return true }
        if visited[node] { return nodes[node] }
        visited[node] = true
...
func main() {
    ...
    fmt.Println(eventualSafeNodes1([][]int{{1,2},{2,3},{5},{0},{5},{},{}})) // [2,4,5,6]
    fmt.Println(eventualSafeNodes1([][]int{{1,2,3,4},{1,2},{3,4},{0,4},{}})) // [4]

    fmt.Println(eventualSafeNodes2([][]int{{1,2},{2,3},{5},{0},{5},{},{}})) // [2,4,5,6]
    fmt.Println(eventualSafeNodes2([][]int{{1,2,3,4},{1,2},{3,4},{0,4},{}})) // [4]
}