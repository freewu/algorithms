...
SELECT
    *,
    CASE 
        WHEN position BETWEEN 1 AND FLOOR((SELECT COUNT(*) FROM t) / 3) + 1 THEN 'Tier 1'
        WHEN position BETWEEN FLOOR((SELECT COUNT(*) FROM t) / 3) + 1  AND FLOOR((SELECT COUNT(*) FROM t) / 1.5) + 1 THEN 'Tier 2'
        ELSE 'Tier 3'
    END AS tier
FROM 
    (
