...
// Conversely, 
// 	if you have k (k >= 3) positive real numbers a1, a2, a3, ..., ak 
...
func largestPerimeter(nums []int) int64 {
	...
	start := nums[0] + nums[1]
	ans := int64(-1)
	// 使用贪婪算法考察每一条边作为最长边时前面的所有边的长度是否超过
	for i := 2; i < len(nums); i = i + 1 {
		if nums[i] < start {
			ans = int64(start + nums[i])
		}
		start += nums[i]
	}
	return ans
...
func main() {
	fmt.Println(largestPerimeter([]int{5,5,5})) // 15
	fmt.Println(largestPerimeter([]int{1,12,1,2,5,50,3})) // 12
	fmt.Println(largestPerimeter([]int{5,5,50})) // -1
}