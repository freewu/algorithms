...
func (this *HitCounter) GetHits(timestamp int) int {
    n, start := len(this.queue), max(0, timestamp - 299) 
    l := sort.Search(n, func(i int) bool { // 在 queue 上找 >=  start 的最小值的下标
        return this.queue[i] >= start
    })
    return n - l
}
...
func main() {
    ...
    fmt.Println(obj)
    // hitCounter.getHits(4);   // get hits at timestamp 4, return 3.
    fmt.Println(obj.GetHits(4)) // 3
    // hitCounter.hit(300);     // hit at timestamp 300.
    obj.Hit(300)
    fmt.Println(obj)
    // hitCounter.getHits(300); // get hits at timestamp 300, return 4.
    fmt.Println(obj.GetHits(300)) // 4
    // hitCounter.getHits(301); // get hits at timestamp 301, return 3.
    fmt.Println(obj.GetHits(301)) // 3
}