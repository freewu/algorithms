...
// best solution
func minPathSumBest(grid [][]int) int {
	// parallel grid which finds the minimum per square
	pg := make([][]int, len(grid))
	for i,_ := range grid {
		pg[i] = make([]int, len(grid[i]))
	}
	for i := 0; i < len(grid); i += 1{
		for j := 0 ; j < len(grid[i]); j += 1 {
			if i == 0 && j == 0 {
				...
				pg[i][j] = pg[i][j-1] + grid[i][j]
			} else if j == 0 {
				pg[i][j] = pg[i-1][j] + grid[i][j]
			} else { // i > 0 && j > 0
				m := pg[i-1][j]
				if pg[i][j-1] < m {
					m = pg[i][j-1]
...
func main() {
	...
	fmt.Printf("minPathSumBest([][]int{ []int{1,3,1},[]int{1,5,1},[]int{4,2,1}}) = %v\n",minPathSumBest([][]int{ []int{1,3,1},[]int{1,5,1},[]int{4,2,1}}))
	fmt.Printf("minPathSumBest([][]int{ []int{1,2,3},[]int{4,5,6} }) = %v\n",minPathSumBest([][]int{  []int{1,2,3},[]int{4,5,6} }))
}
