...
func groupStrings(words []string) []int {
    ...
    var dfs func(mp map[int32]int16, mask int32) int
    dfs = func(mp map[int32]int16, mask int32) int {
        res := 0
        if count, ok := mp[mask]; ok {
            res += int(count)
            delete(mp, mask)
            for i := 0; i < 26; i++ {
                res += dfs(mp, mask^(1<<i))
