...
func numDecodings1(s string) int {
    dp := make(map[int]int)
    var dfs func(s string, start int, dp map[int]int) int 
    dfs = func (s string, start int, dp map[int]int) int {
        if start == len(s) {
            return 1
        }
        if _,ok := dp[start]; ok {
...
func main() {
    ...
    fmt.Printf("numDecodings1(\"12\") = %v\n",numDecodings1("12")) // 2   "AB" (1 2) or "L" (12)
    fmt.Printf("numDecodings1(\"226\") = %v\n",numDecodings1("226")) // 3  "BZ" (2 26), "VF" (22 6), or "BBF" (2 2 6).
    fmt.Printf("numDecodings1(\"06\") = %v\n",numDecodings1("06")) // 0
}
