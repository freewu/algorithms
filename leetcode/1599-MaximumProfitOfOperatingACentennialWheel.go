...
// Example 1:
// <img src="https://assets.leetcode.com/uploads/2020/09/09/wheeldiagram12.png" />
...
//     waiting, curProfit, maxProfit, maxProfitIndex, index := 0, 0, 0, -1, 0
//     for index < len(customers) || waiting > 0 {
...
func minOperationsMaxProfit(customers []int, boardingCost int, runningCost int) int {
    mx, index, nowIndex, nowMoney, nowPeople := 0, -1, 0, 0, 0
    for _, v := range customers {
        nowIndex++
        nowPeople += v
        if nowPeople >= 4 {
            nowPeople -= 4
            nowMoney += 4* boardingCost
        } else {
            nowMoney += nowPeople * boardingCost
            nowPeople = 0
        }
...
func minOperationsMaxProfit1(customers []int, boardingCost, runningCost int) int {
    res, maxProfit, totalProfit, operations, customersCount := -1, 0, 0, 0, 0
