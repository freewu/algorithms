...
func closestCost(baseCosts []int, toppingCosts []int, target int) int {
    ...
    dfs = func(curr int, i int) {
        if abs(curr - target) < abs(res - target) || abs(curr - target) == abs(res - target) && curr < res {
            res = curr
        }
...
func main() {
    ...
    // - Take 0 of topping 1: cost 0 x 4 = 0
    // Total: 7 + 3 + 0 = 10.
    fmt.Println(closestCost([]int{1,7}, []int{3,4}, 10)) // 10
    // Example 2:
    // Input: baseCosts = [2,3], toppingCosts = [4,5,100], target = 18
    // Output: 17
    // Explanation: Consider the following combination (all 0-indexed):
    ...
    // - Take 0 of topping 2: cost 0 x 100 = 0
    // Total: 3 + 4 + 10 + 0 = 17. You cannot make a dessert with a total cost of 18.
    fmt.Println(closestCost([]int{2,3}, []int{4,5,100}, 18)) // 17
    // Example 3:
    // Input: baseCosts = [3,10], toppingCosts = [2,5], target = 9
    // Output: 8
    // Explanation: It is possible to make desserts with cost 8 and 10. Return 8 as it is the lower cost.
    fmt.Println(closestCost([]int{3,10}, []int{2,5}, 9)) // 8
}